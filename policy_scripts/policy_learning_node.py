...
MODULES_PATH = '/mnt/c/Users/keera/Documents/Github/Basic_robot/QtableV1/policy_scripts'
...
from Utils import *
from POLICY_learning import POLICY_net
...
# Q table source file
SOURCE_DIR = CHECKPOINT_DIR + '/POLICY_beta.pth'
...
# Q table source file
parser.add_argument('--SOURCE_DIR', default = SOURCE_DIR, type=str, help='/Checkpoint/DQN_beta.pth')
...
class DQNLearningNode(Node):
    def __init__(self):
        ...
        self.policy_net = POLICY_net(n_observations = 12).to(DEVICE)

        if args_parse.resume:
                self.policy_net.load_state_dict(torch.load(args_parse.SOURCE_DIR))
    ...
    def step(self, action_mode, action_params, queue, angle_max_dist_state):
        """
        ...
        print(f'action-> Action mode: {action_mode} \
              LINEAR_SPEED: {ALPHA*sigmoid(float(action_params[0][0].item()))},  \
              ANGULAR_SPEED: {float(action_params[0][1].item())*(np.pi/2.)}')
        status_rda =  robotDoAction(velPub=self.velPub, 
                                    action=action_mode, 
                                    LINEAR_SPEED=ALPHA*sigmoid(float(action_params[0][0].item())), 
                                    ANGULAR_SPEED=np.tanh(float(action_params[0][1].item()))*(np.pi/2.)
                                    )
    ...
    def timer_callback(self):
            ...
            scores_deque = deque(maxlen=BATCH_SIZE)
            scores = []
            #training....
            if self.episode < args_parse.max_episodes :
                ...
                for t in count():
                    ...
                    action_mode, action_params, log_prob = self.policy_net.act(state)
                    saved_log_prob.append(log_prob)
                    # print(f'action_mode: {action_mode}, action_params: {action_params[0].size()}, log_prob: {log_prob}')
                    queue.append(float(action_mode))
                    # if len(queue) > 6:
                    #     queue.popleft()

                    print(f'queue: {queue}')
                    self.ep_steps += 1
                    observation, reward, terminated, truncated = self.step(action_mode, action_params, list(queue), angle_max_dist_state)
                    ...
                    action = torch.tensor([float(action_mode), ALPHA*sigmoid(float(action_params[0][0].item())), np.tanh(float(action_params[0][1].item()))*(np.pi/2.)], device=DEVICE).view(3)
                    ...
                    if done:### done episode
                        ...
                        # torch.save(self.policy_net.state_dict(), args_parse.SOURCE_DIR)
                        # print(f'saved!!!')
                        break
                scores_deque.append(sum(rewards))
                scores.append(sum(rewards))

                returns = deque(maxlen=int(self.ep_steps))
                
                for t in range(self.ep_steps)[::-1]:
                    disc_return_t = (returns[0] if len(returns)>0.0 else 0.0)
                    returns.appendleft(self.discount_factor*disc_return_t + rewards[t])
                ...
                print(f'\n\t policy_loss: {policy_loss} | LOSS_min: {self.LOSS} \t\n')

                self.optimizer.zero_grad()
                policy_loss.backward()
                self.optimizer.step()

                if (policy_loss != None) and (policy_loss < self.LOSS) or (self.episode % args_parse.max_episodes_before_save == 0) :
                        print('-'*100)
                        torch.save(self.policy_net.state_dict(), args_parse.SOURCE_DIR)
                        print(f'saved!!!')
